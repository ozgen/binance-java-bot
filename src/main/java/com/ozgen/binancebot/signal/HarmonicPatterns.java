...
public class HarmonicPatterns {
    ...
    public static boolean isABCD(double abc, double bcd) {
        return (abc >= 0.382 && abc <= 0.886) && (bcd >= 1.13 && bcd <= 2.618);
    }

    // Check for Bat pattern (uses xab)
    public static boolean isBat(double xab, double abc, double bcd, double xad) {
        return (xab >= 0.382 && xab <= 0.5) && (abc >= 0.382 && abc <= 0.886) &&
                (bcd >= 1.618 && bcd <= 2.618) && (xad <= 0.618 && xad <= 1.0);
    }

    // Check for Gartley pattern (uses xab)
    public static boolean isGartley(double xab, double abc, double bcd, double xad) {
    ...
    public static boolean detectABCDPattern(KlineData x, KlineData a, KlineData b, KlineData c, KlineData d) {
